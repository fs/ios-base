...
- (NSString*)dateStringEurope{
    return [NSString stringWithFormat:@"%02d/%02d/%d", self.day, self.month, self.year];
}
...
#pragma mark - FS extension

- (NSString *)uiStringFromDate {
    if ([self isToday])
        return @"Today";
...
- (NSString *)uiDetailedStringFromDate {
    if ([self isToday]){
        ...
        if (minutes==0){
            ...
            return [NSString stringWithFormat:@"%i minute%@ ago",minutes, minutes>1?@"s":@""];
            
        } else {
            NSInteger hours = [self hoursBeforeDate:[NSDate date]];
            
            return [NSString stringWithFormat:@"%i hour%@ ago",hours, hours>1?@"s":@""];
            
        }
    } else {
        NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
        [formatter setDateFormat:@"hh:mma 'on' MMM dd"];
        NSString *s = [formatter stringFromDate:self];
        return s;
    }
}
